...
transforms = trans.Compose([trans.ToTensor(), trans.Normalize(mean=(0.1307,), std = (0.3081,)),])
...
model = Model(num_input=28*28, num_class=10, num_hidden_layer = 1, num_hidden_unit = [20], activation = 'leaky', dropout = True, drop_rate = 0.5)
...
def eval_model(model, test_loader, epoch, best_acc):
    model.eval()
    num_hit = 0
    ...
    if best_acc < test_accuracy:
        ...
        torch.save(model.state_dict(), './checkpoint/snapchot.pth')
    # ------------ (End) Save model --------------------
    ...
    for tag, value in info.items():
        logger.scalar_summary(tag, value, epoch + 1)
    # ----------Tensorboard Computation -----------------
    return 100. * test_accuracy, best_acc
...
for epoch in range(itr):
    ...
    ts_acc, best_acc = eval_model(model, test_loader, epoch, best_acc)
