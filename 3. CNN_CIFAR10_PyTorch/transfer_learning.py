...
# Loading dataset
train_data = dsets.ImageFolder(root = './hymenoptera_data/train/', transform = train_trans)
val_data = dsets.ImageFolder(root = './hymenoptera_data/val/', transform = val_trans)

# Create DataLoader
train_loader = torch.utils.data.DataLoader(train_data, batch_size = bs, shuffle = True, num_workers = 1)
test_loader = torch.utils.data.DataLoader(val_data, batch_size = bs, shuffle = False, num_workers = 1)
...
optimizer = optim.Adam(params = model.fc.parameters(), lr = learning_rate, weight_decay = wd)
